...
export default function CartPage() {
  ...
  if (items.length === 0) {
    return (
      <div className="min-h-screen bg-canvas  flex flex-col items-center justify-center py-20 px-4">
        ...
        <h2 className="text-2xl font-black text-gray-900  mb-4">ตะกร้าสินค้าของคุณว่างเปล่า</h2>
  ...
  return (
    <div className="w-full min-h-screen bg-canvas  transition-colors duration-300 pt-8 pb-20 px-6 lg:px-12 2xl:px-20 animate-fade-in">
      <h1 className="text-3xl md:text-4xl font-black text-gray-900  mb-10">ตะกร้าสินค้า</h1>

      <div className="flex flex-col lg:flex-row gap-10">
        <div className="flex-1 bg-white  rounded-md p-6 shadow-sm border border-gray-200 ">
          <div className="flex items-center gap-4 border-b border-gray-100  pb-4 mb-4">
            ...
            <span className="font-bold text-gray-900  text-lg">เลือกสินค้าทั้งหมด</span>
          </div>

          <div className="flex flex-col gap-6">
            {items.map((item: any) => (
              <div key={item.productId} className="flex items-center gap-4 p-4 rounded-md border border-gray-100  hover:border-primary transition-colors">
                ...
                <div className="flex flex-col items-end gap-3">
                  ...
                  <div className="flex items-center border border-gray-300  rounded-sm overflow-hidden">
                    <button onClick={() => dispatch(updateQuantity({productId: item.productId, quantity: Math.max(1, item.quantity - 1)}))} className="px-3 py-1 bg-canvas  text-gray-900  hover:bg-gray-200 dark:hover:bg-gray-700">-</button>
                    <span className="px-3 py-1 bg-white  text-gray-900  min-w-10 text-center font-bold">{item.quantity}</span>
                    <button onClick={() => dispatch(updateQuantity({productId: item.productId, quantity: Math.min(item.stock, item.quantity + 1)}))} className="px-3 py-1 bg-canvas  text-gray-900  hover:bg-gray-200 dark:hover:bg-gray-700">+</button>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="lg:w-96 flex flex-col gap-6">
          <div className="bg-white  rounded-md p-6 shadow-sm border border-gray-200 ">
            <h2 className="text-xl font-black text-gray-900  mb-4">โค้ดส่วนลด</h2>
            <div className="flex gap-2">
              <input 
                ...
                className="flex-1 px-4 py-3 rounded-md border border-gray-200  bg-canvas  text-gray-900  focus:ring-2 focus:ring-blue-500 outline-none uppercase"
              />
            </div>
          ...
          <div className="bg-white  rounded-md p-6 shadow-sm border border-gray-200 ">
            <h2 className="text-xl font-black text-gray-900  mb-6">สรุปคำสั่งซื้อ</h2>
