...
const inputStyle = "w-full p-3.5 bg-canvas /50 border border-hairline text-ink text-sm rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all duration-200";
...
export default function NewsTab() {
 ...
 {/* ---------------- ฟอร์มสร้างข่าวสาร ---------------- */}
 <form onSubmit={handleCreateNews} className="bg-canvas p-8 lg:p-10 rounded-md shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-hairline relative overflow-hidden w-full">
 ...
 <input type="file" name="image" accept="image/*" disabled className={`${inputStyle} file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-orange-50 file:text-orange-700 bg-surface-soft cursor-not-allowed`} title="ระบบอัปโหลดรูปกำลังอยู่ในช่วงพัฒนา" />
 </div>
 </div>
 ...
 {news.map(n => (
 <div key={String(n.id)} className="bg-canvas rounded-md border border-hairline shadow-sm hover:shadow-md transition-all overflow-hidden flex flex-col relative group">
 ...
 <div className="p-4 bg-canvas/80 /80 border-t border-hairline grid grid-cols-2 gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
 ...
 <div className="sticky top-0 bg-canvas z-10 px-8 py-5 border-b border-hairline flex justify-between items-center">
 ...
 <input type="file" name="image" accept="image/*" disabled className={`${inputStyle} bg-surface-soft cursor-not-allowed`} />
 </div>
 </div>
 </div>

 <div className="p-6 border-t border-hairline bg-canvas /50 flex justify-end gap-3 rounded-b-3xl">
 <button type="button" onClick={() => setEditingNews(null)} className="px-6 py-3 bg-canvas border border-hairline text-ink rounded-md font-bold hover:bg-surface-soft dark:hover:bg-gray-600 transition-colors">
 ยกเลิก
 </button>
