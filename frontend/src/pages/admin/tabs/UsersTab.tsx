...
export default function UsersTab() {
 ...
 if (loading) {
 return <div className="p-8 text-center text-muted ">กำลังโหลดข้อมูลผู้ใช้งาน...</div>;
 }

 return (
 <div className="bg-canvas rounded-md shadow-sm border border-hairline p-6">
 <h2 className="text-xl font-bold text-ink mb-6">จัดการผู้ใช้งาน สิทธิ์ & การเงิน</h2>
 ...
 u.role === 'owner' ? 'bg-orange-100 text-orange-700 /30 ' :
 u.role === 'center' ? 'bg-indigo-100 text-indigo-700 /30 ' :
 u.role === 'rider' ? 'bg-pink-100 text-pink-700 /30 ' :
 ...
 className="px-3 py-1.5 bg-blue-100 hover:bg-blue-200 /30 dark:hover:bg-blue-900/50 text-blue-700 rounded-sm text-sm font-medium transition-colors"
 >
 จัดการสิทธิ์
 </button>
 <button 
 onClick={() => openWalletModal(u)} 
 className="px-3 py-1.5 bg-yellow-100 hover:bg-yellow-200 /30 dark:hover:bg-yellow-900/50 text-yellow-700 rounded-sm text-sm font-medium transition-colors"
 >
 จัดการเงิน
 </button>
 </td>
 ...
 <h3 className="text-xl font-bold text-ink mb-4">ปรับปรุงยอดเงิน</h3>
 ...
 className="w-full px-4 py-3 border border-hairline rounded-md bg-canvas text-ink text-lg font-bold focus:ring-2 focus:ring-blue-500 outline-none" 
 />
 </div>
 
 <div className="flex justify-end gap-3">
 <button type="button" onClick={() => setWalletModal(false)} className="px-4 py-2 rounded-md bg-surface-soft hover:bg-surface-soft dark:hover:bg-gray-600 text-ink font-medium transition-colors">
 ยกเลิก
 </button>
 ...
 <div className="fixed inset-0 z-50 flex items-center justify-center bg-canvas/50 backdrop-blur-sm p-4">
 <div className="bg-canvas rounded-md w-full max-w-sm shadow-2xl p-6">
 <h3 className="text-xl font-bold text-ink mb-4">แก้ไขสิทธิ์ผู้ใช้งาน</h3>
 <p className="text-sm text-muted mb-6">บัญชี: {selectedUser.email || selectedUser.username}</p>
 ...
 className="w-full px-4 py-3 border border-hairline rounded-md bg-canvas text-ink text-base focus:ring-2 focus:ring-blue-500 outline-none"
 >
 ...
 <div className="flex justify-end gap-3">
 <button type="button" onClick={() => setRoleModal(false)} className="px-4 py-2 rounded-md bg-surface-soft hover:bg-surface-soft dark:hover:bg-gray-600 text-ink font-medium transition-colors">
 ยกเลิก
 </button>
 <button type="submit" className="px-4 py-2 rounded-md bg-primary hover:bg-primary-active text-white font-medium shadow-md transition-colors">
 บันทึกสิทธิ์
