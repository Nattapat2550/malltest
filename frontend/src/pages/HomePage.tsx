    ...
    <div className="w-full overflow-x-hidden bg-canvas  pb-20 transition-colors duration-300">
        ...
        <div className="w-full px-6 lg:px-12 2xl:px-20 py-24 lg:py-32 relative z-10 flex flex-col md:flex-row items-center justify-between gap-12">
          <div className="text-center md:text-left flex-1">
            ...
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-semibold mb-6 leading-tight tracking-tighter">
              ...
              <span className="text-gray-400 ">Shopping</span> Journey
            </h1>
      ...
      {isPromosLoading ? (
        <div className="w-full px-6 lg:px-12 2xl:px-20 mt-16">
          <div className="h-10 w-48 bg-gray-200  rounded-sm animate-pulse mb-6"></div>
          <div className="flex gap-4 overflow-x-auto pb-4">
            {[1,2,3,4].map(i => <div key={i} className="min-w-70 h-40 bg-gray-200  rounded-md animate-pulse"></div>)}
          </div>
        </div>
        ...
        <div className="w-full px-6 lg:px-12 2xl:px-20 mt-16">
          ...
          <div className="flex gap-4 overflow-x-auto pb-4 snap-x">
            {activePromos.map((promo: any) => {
              ...
              return (
                <div key={promo.id} className="min-w-70 bg-surface-soft  border border-gray-200  rounded-md p-6 text-gray-900  shadow-sm snap-center relative overflow-hidden transition-transform hover:-translate-y-1">
                  {promo.shop_id && <span className="absolute top-4 right-4 text-xs font-semibold bg-gray-200  px-3 py-1 rounded-full">โค้ดร้านค้า</span>}
                  <h3 className="text-3xl font-semibold mb-2 tracking-tight">{promo.code}</h3>
                  <p className="text-sm font-medium mb-4 text-gray-500 ">{promo.description}</p>
                  <p className="text-xs mb-5 bg-gray-200  inline-block px-3 py-1.5 rounded-sm font-medium">ขั้นต่ำ {promo.min_purchase.toLocaleString()}฿</p>
                  <button 
            ...
            {documents.map((d: any) => (
              <div key={d.id} className="group apple-glass rounded-[2rem] overflow-hidden flex flex-col h-full hover:shadow-2xl transition-all duration-300">
                ...
                <div className="p-6 flex flex-col flex-1 bg-white/50 /50">
                  <h3 className="text-xl font-semibold mb-4 text-gray-900  leading-snug line-clamp-2 tracking-tight">{d.title}</h3>
                  <div className="mt-auto pt-4">
                    <Link to={`/documents/${d.id}`} className="flex justify-center items-center w-full bg-surface-soft hover:bg-gray-200  dark:hover:bg-gray-700 text-gray-900  font-semibold py-3.5 rounded-full transition-colors duration-300">
                      อ่านเพิ่มเติม
                    </Link>
                  </div>
                </div>
