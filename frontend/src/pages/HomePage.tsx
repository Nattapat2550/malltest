...
export default function HomePage() {
 ...
 <div className="relative bg-canvas text-ink overflow-hidden border-b border-hairline ">
 ...
 <span className="text-muted ">Shopping</span> Journey
 </h1>
 ...
 <div className="h-10 w-48 bg-surface-soft rounded-sm animate-pulse mb-6"></div>
 <div className="flex gap-4 overflow-x-auto pb-4">
 {[1,2,3,4].map(i => <div key={i} className="min-w-70 h-40 bg-surface-soft rounded-md animate-pulse"></div>)}
 </div>
 </div>
 ...
 <Tag className="text-muted" size={32} /> โปรโมชันเด็ด
 </h2>
 <Link to="/promotions" className="text-muted hover:text-ink dark:hover:text-white font-medium flex items-center transition-colors">ดูทั้งหมด <ChevronRight size={20}/></Link>
 </div>
 <div className="flex gap-4 overflow-x-auto pb-4 snap-x">
 {activePromos.map((promo: any) => {
 const isCollected = myPromos.some((mp: any) => mp.id === promo.id || mp.promotion_id === promo.id);
 return (
 <div key={promo.id} className="min-w-70 bg-surface-soft border border-hairline rounded-md p-6 text-ink shadow-sm snap-center relative overflow-hidden transition-transform hover:-translate-y-1">
 {promo.shop_id && <span className="absolute top-4 right-4 text-xs font-semibold bg-surface-soft px-3 py-1 rounded-full">โค้ดร้านค้า</span>}
 <h3 className="text-3xl font-semibold mb-2 tracking-tight">{promo.code}</h3>
 <p className="text-sm font-medium mb-4 text-muted ">{promo.description}</p>
 <p className="text-xs mb-5 bg-surface-soft inline-block px-3 py-1.5 rounded-sm font-medium">ขั้นต่ำ {promo.min_purchase.toLocaleString()}฿</p>
 <button 
 onClick={() => handleCollectPromo(promo.id)}
 disabled={isCollected}
 className={`w-full py-3 rounded-full font-semibold text-sm transition-all shadow-sm ${isCollected ? 'bg-surface-soft cursor-not-allowed text-muted' : 'bg-zinc-950 text-white hover:scale-105'}`}
 >
 ...
 <div className="w-full h-62.5 md:h-100 lg:h-112.5 rounded-[2rem] bg-surface-soft animate-pulse"></div>
 </div>
 ) : carousels.length > 0 && (
 <div className="w-full px-6 lg:px-12 2xl:px-20 mt-10">
 <div className="relative w-full h-62.5 md:h-100 lg:h-112.5 rounded-[2rem] overflow-hidden shadow-2xl bg-canvas border border-hairline ">
 ...
 <Link to={`/documents/${d.id}`} className="flex justify-center items-center w-full bg-surface-soft hover:bg-surface-soft dark:hover:bg-gray-700 text-ink font-semibold py-3.5 rounded-full transition-colors duration-300">
 อ่านเพิ่มเติม
 </Link>
 </div>
 </div>
