...
const MyOrdersPage: React.FC = () => {
  ...
  return (
    <div className="container mx-auto p-4 max-w-4xl min-h-screen">
      ...
      {orders.length === 0 ? (
        <div className="text-center p-8 bg-white  rounded-sm shadow-sm border  ">
          คุณยังไม่มีประวัติคำสั่งซื้อ
        </div>
      ) : (
        <div className="space-y-6">
          {orders.map((order) => (
            <div key={order.id} className="bg-white  p-5 rounded-md shadow-sm border-2 border-blue-100 hover:border-blue-300 /30 dark:hover:border-blue-800 hover:shadow-md transition-all">
              ...
              <div className="flex gap-3 mt-4">
                <button onClick={() => setSelectedOrder(order)} className="flex-1 py-2 px-4 bg-surface-soft  text-gray-800  rounded-md text-sm font-semibold">ดูรายละเอียด</button>
      ...
      {reviewModal && (
        <div className="fixed inset-0 bg-canvas/60 backdrop-blur-sm flex items-center justify-center z-60 p-4">
          <div className="bg-white  w-full max-w-md rounded-md overflow-hidden shadow-2xl p-6">
      ...
      {selectedOrder && (
        <div className="fixed inset-0 bg-canvas/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white  w-full max-w-lg rounded-md overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
      ...
      {trackingOrder && (
        <div className="fixed inset-0 bg-canvas/60 flex items-center justify-center z-50 p-4">
            <div className="bg-white  w-full max-w-lg rounded-md p-6">
