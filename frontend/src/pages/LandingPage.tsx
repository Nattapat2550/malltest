...
const LandingPage = () => {
 ...
 <Link 
 to="/login" 
 className="flex-1 bg-canvas border-2 border-slate-200 hover:border-primary dark:hover:border-primary text-slate-900 font-bold py-3 px-6 rounded-md shadow-sm transition-all text-center flex items-center justify-center"
 >
 เข้าสู่ระบบ
 </Link>
 </div>
 ...
 <div className="relative w-full h-62.5 md:h-100 lg:h-112.5 rounded-md overflow-hidden shadow-xl bg-canvas border border-slate-200 ">
 {carousels.map((c, idx) => (
 <div
 key={c.id}
 ...
 {documents.map(d => (
 <div key={d.id} className="group bg-canvas rounded-md shadow-sm overflow-hidden border border-slate-200 flex flex-col h-full transition-all duration-300 hover:shadow-2xl hover:-translate-y-2">
