...
interface Product { id: string; sku: string; name: string; description: string; price: number; stock: number; image_url: string; }
...
export default function ShopPage() {
 ...
 <h2 className="text-2xl md:text-3xl font-black text-orange-500 border-b-2 border-orange-500 pb-2 inline-block">
 ...
 className="w-full pl-11 pr-4 py-3.5 rounded-md border border-slate-200 bg-canvas text-slate-900 focus:ring-2 focus:ring-orange-500 outline-none shadow-sm transition-all"
 />
 ...
 <div className="text-center py-24 bg-canvas rounded-md border border-slate-200 shadow-sm">
 ...
 <Link to={`/products/${p.id}`} key={p.id} className="group bg-canvas rounded-md shadow-sm border border-slate-200 overflow-hidden hover:shadow-2xl hover:shadow-orange-500/10 hover:-translate-y-1.5 transition-all duration-300 flex flex-col h-full">
 ...
 <div className="w-full h-full flex items-center justify-center text-slate-400"><span className="font-bold tracking-widest text-sm uppercase">NO IMAGE</span></div>
 )}
