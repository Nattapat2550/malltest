...
export const ProductComments: React.FC<{ productId: string }> = ({ productId }) => {
 ...
 <h3 className="text-2xl font-black text-ink ">รีวิวจากผู้ใช้งาน ({comments.length})</h3>
 </div>

 <div className="space-y-6">
 {comments.length === 0 ? (
 <p className="text-center py-10 text-muted italic">ยังไม่มีรีวิวสำหรับสินค้านี้</p>
 ) : (
 comments.map((comment) => (
 <div key={comment.id} className="border-b border-hairline pb-6 last:border-0">
 ...
 <button onClick={() => setEditingId(null)} className="text-muted px-4 py-1.5 text-sm">ยกเลิก</button>
 </div>
 </div>
 ) : (
 // --- โหมดแสดงผลปกติ ---
 ...
 <p className="text-ink font-bold">{comment.message}</p>
 </div>
