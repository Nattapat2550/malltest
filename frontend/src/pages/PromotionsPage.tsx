...
export default function PromotionsPage() {
 ...
 <button onClick={() => setFilter('all')} className={filter === 'all' ? "px-6 py-2 rounded-full font-bold transition-all bg-primary text-white shadow-md" : "px-6 py-2 rounded-full font-bold transition-all bg-canvas text-primary border border-blue-200 "}>ทั้งหมด</button>
 <button onClick={() => setFilter('platform')} className={filter === 'platform' ? "px-6 py-2 rounded-full font-bold transition-all bg-primary text-white shadow-md" : "px-6 py-2 rounded-full font-bold transition-all bg-canvas text-primary border border-blue-200 "}>โปรจากระบบ</button>
 <button onClick={() => setFilter('shop')} className={filter === 'shop' ? "px-6 py-2 rounded-full font-bold transition-all bg-primary text-white shadow-md" : "px-6 py-2 rounded-full font-bold transition-all bg-canvas text-primary border border-blue-200 "}>โค้ดร้านค้า</button>
 </div>
 ...
 <div className="absolute -right-4 -top-4 w-24 h-24 bg-canvas/20 rounded-full blur-2xl"></div>
 {isShop && <span className="absolute top-4 right-4 bg-canvas/20 px-2 py-1 rounded text-xs font-bold">โค้ดร้านค้า</span>}
 ...
 className={`w-full py-3 rounded-md font-bold text-sm transition-all shadow-md ${isCollected ? 'bg-canvas/30 cursor-not-allowed text-white' : 'bg-canvas text-slate-900 hover:scale-105'}`}
 >
