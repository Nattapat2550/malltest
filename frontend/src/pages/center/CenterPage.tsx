...
export default function CenterPage() {
 ...
 default: return 'bg-surface-soft text-ink border-hairline';
 }
 };
 ...
 <h1 className="text-3xl font-black text-ink mb-2">Delivery Center</h1>
 <p className="text-muted ">ศูนย์: <span className="font-bold text-purple-600">{centerInfo.name || 'ยังไม่ได้ตั้งชื่อศูนย์'} <br className="md:hidden" /><span className="text-xs break-all">(ID: {centerInfo.id})</span></span></p>
 ...
 activeTab === tab ? 'border-purple-600 text-purple-600' : 'border-transparent text-muted hover:text-ink dark:hover:text-white'
 }`}
 >
 ...
 <h2 className="text-xl font-bold text-ink ">จัดการพัสดุในระบบ</h2>
 ...
 <thead>
 <tr className="bg-canvas text-muted border-b ">
 ...
 {shipments.length === 0 ? <tr><td colSpan={6} className="p-6 text-center text-muted">ไม่มีพัสดุที่เกี่ยวข้องกับศูนย์นี้</td></tr> :
 shipments.map((s, idx) => (
 ...
 <td className="p-4 font-mono text-xs text-ink ">#{s.shipment_id}</td>
 <td className="p-4 text-xs font-mono text-muted">#{s.order_id}</td>
 ...
 {activeTab === 'riders' && (
 <div className="bg-canvas rounded-md p-6 lg:p-8 shadow-sm">
 <h2 className="text-xl font-bold text-ink mb-6">จัดการ Rider ประจำศูนย์</h2>
 ...
 <tbody className="divide-y divide-hairline ">
 {riders.length === 0 ? <tr><td colSpan={3} className="p-6 text-center text-muted">ยังไม่มี Rider ในศูนย์</td></tr> : 
 riders.map(r => (
 <tr key={r.id}>
 ...
 {activeTab === 'profile' && (
 <div className="bg-canvas rounded-md p-6 lg:p-8 shadow-sm max-w-md">
 <h2 className="text-xl font-bold text-ink mb-6">แก้ไขข้อมูลศูนย์กระจายสินค้า</h2>
 <form onSubmit={handleSaveProfile} className="space-y-4">
 <div>
 <label className="block text-sm font-bold text-ink mb-1">ชื่อศูนย์</label>
 ...
 <div className="bg-canvas rounded-md w-full max-w-md p-6">
 ...
 <button type="button" onClick={() => setShowUpdateModal(false)} className="px-4 py-2 bg-surface-soft rounded-sm font-bold">ยกเลิก</button>
