...
export default function DocumentDetailsPage() {
 ...
 <Link to="/" className="inline-flex items-center text-primary font-bold hover:underline mb-8 bg-canvas px-5 py-2.5 rounded-full border border-hairline shadow-sm transition-colors">
 &larr; กลับหน้าหลัก
 </Link>

 {/* ภาพปก */}
 {doc.cover_image && (
 <div className="w-full h-75 md:h-125 rounded-md overflow-hidden mb-10 shadow-lg border border-hairline bg-canvas ">
 ...
 <div 
 className="prose prose-lg dark:prose-invert max-w-none mb-16 text-ink leading-relaxed bg-canvas p-8 rounded-md border border-hairline shadow-sm"
 ...
 {gallery.map((url, idx) => (
 <div key={idx} className="group rounded-md overflow-hidden shadow-sm bg-canvas border border-hairline hover:shadow-xl hover:border-primary/30 transition-all cursor-pointer">
 <img 
 src={url} 
 alt={`Gallery ${idx + 1}`} 
